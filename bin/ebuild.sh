...
save_ebuild_env() {
	(
		...
		# Prefix additions
		unset DEFAULT_PATH DISTDIR EROOT ED FILESDIR HOME INFOPATH MANPATH

		# Paranoia
		unset HOSTTYPE MACHTYPE CVS_RSH EUID GCC_SPECS \
			GENTOO_MIRRORS GROUP HOST HOSTNAME PORTAGE_GROUP PORTAGE_USER \
			SHELLOPTS SYNC UID dir srcdir STY SSH_AGENT_PID TERM_PROGRAM \
			TERM_PROGRAM_VERSION USER SSH_AUTH_SOCK \
			__CF_USER_TEXT_ENCODING ECHANGELOG_USER TERMINFO VENDOR \
			LOGNAME SECURITYSESSIONID REMOTEHOST MAIL
