...
def _get_global(k):
    ...
    if k == "secpass":
        ...
        try:
            ...
            writemsg(
                '!!! PORTAGE_BUILD_USER refers to nonexistent user "%s"\n'
                % _get_global("_build_user"),
                noiselevel=-1,
            )
            build_uid = 0
        ...
        try:
            ...
            writemsg(
                '!!! PORTAGE_BUILD_GROUP refers to nonexistent group "%s"\n'
                % _get_global("_build_group"),
                noiselevel=-1,
            )
            build_gid = 0
        ...
        if secpass >= 2:
            ...
            cmd = (
                ...
                for x in ("id", "-G", _get_global("_build_user"))
            )
