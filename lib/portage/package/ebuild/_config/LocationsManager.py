...
import portage
from portage import os_unicode_fs, eapi_is_supported
...
class LocationsManager:
    def __init__(
        ...
        if self.eprefix is None:
            ...
            if self.eprefix == os_unicode_fs.sep:
                self.eprefix = ""

        if self.config_root is None:
            self.config_root = portage.const.EPREFIX + os_unicode_fs.sep

        self.config_root = (
            normalize_path(
                os_unicode_fs.path.abspath(self.config_root or os_unicode_fs.sep)
            ).rstrip(os_unicode_fs.sep)
            + os_unicode_fs.sep
        )

        self._check_var_directory("PORTAGE_CONFIGROOT", self.config_root)
        self.abs_user_config = os_unicode_fs.path.join(
            self.config_root, USER_CONFIG_PATH
        )
        ...
        if self.sysroot is None:
            ...
            self.sysroot = (
                normalize_path(
                    os_unicode_fs.path.abspath(self.sysroot or os_unicode_fs.sep)
                ).rstrip(os_unicode_fs.sep)
                + os_unicode_fs.sep
            )

        self.esysroot = (
            self.sysroot.rstrip(os_unicode_fs.sep) + self.eprefix + os_unicode_fs.sep
        )
    ...
    def load_profiles(self, repositories, known_repository_paths):
        known_repository_paths = set(
            os_unicode_fs.path.realpath(x) for x in known_repository_paths
        )
        ...
        if self.profile_path:
            try:
                self._addProfile(
                    os_unicode_fs.path.realpath(self.profile_path),
                    repositories,
                    known_repos,
                    (),
                )
        ...
        if self._user_config and self.profiles:
            custom_prof = os_unicode_fs.path.join(self.config_root, CUSTOM_PROFILE_PATH)
            if os_unicode_fs.path.exists(custom_prof):
                ...
                self.profiles_complex.append(
                    _profile_node(
                        ...
                        read_corresponding_eapi_file(
                            custom_prof + os_unicode_fs.sep, default=None
                        ),
                        True,
    ...
    def _addProfile(self, currentPath, repositories, known_repos, previous_repos):
        current_abs_path = os_unicode_fs.path.abspath(currentPath)
        ...
        eapi_file = os_unicode_fs.path.join(currentPath, "eapi")
        eapi = eapi or "0"
        f = None
        try:
            f = io.open(
                eapi_file.encode(encoding="utf-8", errors="strict"),
                mode="r",
                encoding="utf-8",
                errors="replace",
            )
            ...
            if not eapi_is_supported(eapi):
                raise ParseError(
                    ...
                    % (
                        eapi,
                        os_unicode_fs.path.realpath(eapi_file),
                    )
                )
        finally:
            if f is not None:
        ...
        if intersecting_repos:
            ...
            allow_parent_colon = any(
                x in _allow_parent_colon for x in layout_data["profile-formats"]
            )
        ...
        if compat_mode:
            offenders = _PORTAGE1_DIRECTORIES.intersection(
                os_unicode_fs.listdir(currentPath)
            )
            offenders = sorted(
                x
                for x in offenders
                if os_unicode_fs.path.isdir(os_unicode_fs.path.join(currentPath, x))
            )
            if offenders:
                warnings.warn(
                    _(
        ...
        if exists_raise_eaccess(parentsFile):
            ...
            for parentPath in parents:
                ...
                if (
                    ...
                    parentPath = os_unicode_fs.path.realpath(parentPath)

                if exists_raise_eaccess(parentPath):
                    self._addProfile(
                        parentPath, repositories, known_repos, intersecting_repos
                    )
                else:
                    raise ParseError(
    ...
    def set_root_override(self, root_overwrite=None):
        ...
        self.target_root = self.target_root or os_unicode_fs.sep

        self.target_root = (
            normalize_path(os_unicode_fs.path.abspath(self.target_root)).rstrip(
        ...
        self.eroot = (
            self.target_root.rstrip(os_unicode_fs.sep)
            + self.eprefix
            + os_unicode_fs.sep
        )
        ...
        if portage.const.EPREFIX:
            self.global_config_path = os_unicode_fs.path.join(
