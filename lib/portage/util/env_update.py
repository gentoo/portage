...
def env_update(
    ...
    and ld.so.conf. Finally, run ldconfig. When ldconfig is
...
def _env_update(makelinks, target_root, prev_mtimes, contents, env, writemsg_level):
    ...
    if grabfile(ldsoconf_path) != newld:
        ...
        if eprefix:
            ...
            bfd_ldsoconf_path = os.path.join(eroot, "usr", "etc", "ld.so.conf")
            ensure_dirs(os.path.dirname(bfd_ldsoconf_path), mode=0o755)
            with atomic_ofstream(bfd_ldsoconf_path) as myfd:
                myfd.write(
                    "# ld.so.conf autogenerated by env-update; make all changes to\n"
                    f"# contents of {eprefix}/etc/env.d directory.\n"
    ...
    ldsoconfd_paths = set(getlibpaths(eroot, env=env))

    current_time = int(time.time())
    mtime_changed = False

    lib_dirs = set()
    for lib_dir in set(specials["LDPATH"]) | potential_lib_dirs | ldsoconfd_paths:
