...
def _env_update(makelinks, target_root, prev_mtimes, contents, env, writemsg_level):
    ...
    if not prelink_capable:
        etc_path = Path(eroot, "etc")
        prelink_files = (
            etc_path.joinpath("prelink.conf.d", "portage.conf"),
            etc_path.joinpath("prelink.cache"),
            etc_path.joinpath("prelink.conf"),
        )
        removable_files = (file for file in prelink_files if file.is_file())
        for removable_file in removable_files:
            removable_file.unlink()
        try:
            etc_path.joinpath("prelink.conf.d").rmdir()
