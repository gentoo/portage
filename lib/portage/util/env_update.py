...
def _env_update(makelinks, target_root, prev_mtimes, contents, env, writemsg_level):
    ...
    if oldld != newld:
        ...
        if eprefix:
            # The runtime linker needs ld.so.conf paths to be prefixed, but the
            # bfd linker needs them to be unprefixed, so write an alternative
            # ld.so.conf file for the latter. See bug #892549.
