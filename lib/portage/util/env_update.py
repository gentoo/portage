    ...
    if vardbapi is None:
        if isinstance(env, config):
            ...
            if target_root is None:
                ...
                target_eroot = os_unicode_fs.path.join(
                    target_root, eprefix.lstrip(os_unicode_fs.sep)
                )
                target_eroot = (
                    target_eroot.rstrip(os_unicode_fs.sep) + os_unicode_fs.sep
                )
...
def _env_update(makelinks, target_root, prev_mtimes, contents, env, writemsg_level):
    ...
    eprefix_lstrip = eprefix.lstrip(os_unicode_fs.sep)
    eroot = (
        normalize_path(os_unicode_fs.path.join(target_root, eprefix_lstrip)).rstrip(
            os_unicode_fs.sep
        )
        + os_unicode_fs.sep
    )
    envd_dir = os_unicode_fs.path.join(eroot, "etc", "env.d")
    ...
    for x in fns:
        file_path = os_unicode_fs.path.join(envd_dir, x)
        try:
    ...
    if prelink_capable:
        prelink_d = os_unicode_fs.path.join(eroot, "etc", "prelink.conf.d")
        ensure_dirs(prelink_d)
        newprelink = atomic_ofstream(os_unicode_fs.path.join(prelink_d, "portage.conf"))
        newprelink.write(
    ...
    for lib_dir in set(specials["LDPATH"]) | potential_lib_dirs:
        x = os_unicode_fs.path.join(eroot, lib_dir.lstrip(os_unicode_fs.sep))
        try:
            newldpathtime = os_unicode_fs.stat(x)[stat.ST_MTIME]
        ...
        if newldpathtime == current_time:
            ...
            newldpathtime -= 1
            os_unicode_fs.utime(x, (newldpathtime, newldpathtime))
    ...
    if makelinks and not mtime_changed and contents is not None:
        ...
        for mypath, mydata in contents.items():
            if mydata[0] not in ("obj", "sym"):
                continue
            head, tail = os_unicode_fs.path.split(mypath)
            if head in lib_dirs:
    ...
    if (
        ...
        ldconfig = os_unicode_fs.path.join(eroot, "sbin", "ldconfig")

    if ldconfig is None:
        pass
    elif not (
        os_unicode_fs.access(ldconfig, os_unicode_fs.X_OK)
        and os_unicode_fs.path.isfile(ldconfig)
    ):
        ldconfig = None
    ...
    if makelinks and ldconfig:
        ...
        if ostype == "Linux" or ostype.lower().endswith("gnu"):
            ...
            os_unicode_fs.system("cd / ; %s -X -r '%s'" % (ldconfig, target_root))
    ...
    profile_env_path = os_unicode_fs.path.join(eroot, "etc", "profile.env")
    ...
    systemd_environment_dir = os_unicode_fs.path.join(eroot, "etc", "environment.d")
    os_unicode_fs.makedirs(systemd_environment_dir, exist_ok=True)

    systemd_gentoo_env_path = os_unicode_fs.path.join(
    ...
    outfile = atomic_ofstream(os_unicode_fs.path.join(eroot, "etc", "csh.env"))
