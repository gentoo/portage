    ...
    def update(self, setname, options):
        ...
        if not setname in self.psets:
            ...
            while setname in parser.sections():
                setname = f"{random.randint(0, 10**10):08d}"
            ...
            if parser.has_option(section, "multiset") and parser.getboolean(
                ...
                self.errors.append(
                    _(
                        f"Invalid request to reconfigure set '{setname}' "
                        f"generated by multiset section '{section}'"
                    )
                )
                return
            for k, v in options.items():
    ...
    def _parse(self, update=False):
        ...
        for sname in parser.sections():
            ...
            try:
                ...
                try:
                    setclass = load_mod(f"portage._sets.{classname}")
                except (ImportError, AttributeError):
                    self.errors.append(
                        _(f"Could not import '{classname}' for section '{sname}'")
                    )
                    continue
            ...
            if parser.has_option(sname, "multiset") and parser.getboolean(
                ...
                if hasattr(setclass, "multiBuilder"):
                    ...
                    try:
                        ...
                        self.errors.append(
                            _(f"Configuration error in section '{sname}': {e}")
                        )
                        continue
                    for x in newsets:
                        if x in self.psets and not update:
                            self.errors.append(
                                _(
                                    f"Redefinition of set '{x}' (sections: '{self.psets[x].creator}', '{sname}')"
                                )
                            )
                    ...
                    self.errors.append(
                        _(
                            "Section '{sname}' is configured as multiset, but '{classname}' "
                ...
                if setname in self.psets and not update:
                    self.errors.append(
                        _(
                            f"Redefinition of set '{setname}' (sections: '{self.psets[setname].creator}', '{sname}')"
                        )
                    )
