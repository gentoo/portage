...
"""
    ...
    def _iter_metadata_cache_entries(self, repo_name, ebuild_paths):
        """
        Yield (key, path) for each ebuild, where key identifies its
        repository, cpv and content in the shared cache, and path is its
        entry in the depcachedir.
        """
        ...
        for ebuild_path in ebuild_paths:
            cat = os.path.basename(os.path.dirname(os.path.dirname(ebuild_path)))
            ...
            yield key, os.path.join(dep_repo_dir, cat, pf)
    ...
    def _create_ebuild_manifests(self, ebuilds):
        ...
        for repo_name in self._repositories:
            ...
            # Seed the depcachedir with entries generated by earlier
            # playgrounds. Stale ones fail validation and are regenerated.
            uncached = []
            if cache_dir is not None:
                for key, entry_path in self._iter_metadata_cache_entries(
