...
class AsynchronousLockTestCase(TestCase):
        ...
        try:
            ...
            for force_async in (True, False):
                for force_dummy in (
                    ...
                    async_lock.start()
                    self.assertEqual(async_lock.wait(), os_unicode_fs.EX_OK)
                    self.assertEqual(async_lock.returncode, os_unicode_fs.EX_OK)
    ...
    def _testAsynchronousLockWait(self):
        ...
        try:
            ...
            lock1.start()
            self.assertEqual(lock1.wait(), os_unicode_fs.EX_OK)
            self.assertEqual(lock1.returncode, os_unicode_fs.EX_OK)
            ...
            self.assertEqual(lock2.wait(), os_unicode_fs.EX_OK)
            self.assertEqual(lock2.returncode, os_unicode_fs.EX_OK)
    ...
    def _testAsynchronousLockWaitCancel(self):
        ...
        try:
            ...
            lock2.cancel()
            self.assertEqual(lock2.wait() == os_unicode_fs.EX_OK, False)
            self.assertEqual(lock2.returncode == os_unicode_fs.EX_OK, False)
    ...
    def _testAsynchronousLockWaitKill(self):
        ...
        try:
            ...
            os_unicode_fs.kill(lock2._imp._proc.pid, signal.SIGTERM)
            self.assertEqual(lock2.wait() == os_unicode_fs.EX_OK, False)
            self.assertEqual(lock2.returncode == os_unicode_fs.EX_OK, False)
            self.assertEqual(lock2.returncode is None, False)
            scheduler.run_until_complete(lock1.async_unlock())
