...
class Package(Task):
                        ...
                        if atom.slot_operator_built:
                            e = InvalidDependString(
                                _(
                                    ...
                                    f'"built" atom syntax: {atom.unevaluated_atom}'
                                )
                            )
                            self._metadata_exception(k, e)
        ...
        if v and not self.built:
            if not _get_eapi_attrs(eapi).required_use:
                ...
                try:
                    ...
                    self._invalid_metadata(f"{k}.syntax", f"{k}: {e}")
    ...
    def __str__(self):
        ...
        output = portage.output.colorize(
            cpv_color,
            (
                f"{self.cpv}{build_id_str}{_slot_separator}{self.slot}/"
                f"{self.sub_slot}{_repo_separator}{self.repo}"
            ),
        )
        s = f"({output}, {self.type_name}"
