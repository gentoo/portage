...
class AbstractEbuildProcess(SpawnProcess):
    ...
    def _start(self):
        ...
        # doesn't exist yet.
        if need_builddir and not os_unicode_fs.path.isdir(
        ...
        if (
            os_unicode_fs.geteuid() == 0
            and platform.system() == "Linux"
            and "cgroup" in self.settings.features
            and self.phase not in _global_pid_phases
        ):
            cgroup_root = "/sys/fs/cgroup"
            cgroup_portage = os_unicode_fs.path.join(cgroup_root, "portage")

            try:
                # cgroup tmpfs
                if not os_unicode_fs.path.ismount(cgroup_root):
                    # we expect /sys/fs to be there already
                    if not os_unicode_fs.path.isdir(cgroup_root):
                        os_unicode_fs.mkdir(cgroup_root, 0o755)
                ...
                # portage subsystem
                if not os_unicode_fs.path.ismount(cgroup_portage):
                    if not os_unicode_fs.path.isdir(cgroup_portage):
                        os_unicode_fs.mkdir(cgroup_portage, 0o755)
                    subprocess.check_call(
                        [
                            "mount",
                            "-t",
                    ...
                    with open(
                        os_unicode_fs.path.join(cgroup_portage, "release_agent"), "w"
                    ) as f:
                        f.write(
                    ...
                    with open(
                        os_unicode_fs.path.join(cgroup_portage, "notify_on_release"),
                        "w",
                    ) as f:
                        f.write("1")
                else:
                    ...
                    # itself.
                    release_agent = os_unicode_fs.path.join(
                    ...
                    if release_agent_path is None or not os_unicode_fs.path.exists(
                        release_agent_path
                    ):
        ...
        if self._enable_ipc_daemon:
            ...
            self.settings.pop("PORTAGE_IPC_DAEMON", None)
            if self.phase not in self._phases_without_builddir:
                exit_file = os_unicode_fs.path.join(
                    self.settings["PORTAGE_BUILDDIR"], ".exit_status"
                )
                self.settings["PORTAGE_EBUILD_EXIT_FILE"] = exit_file
                try:
                    os_unicode_fs.unlink(exit_file)
                except OSError:
                    if os_unicode_fs.path.exists(exit_file):
    ...
    def _init_ipc_fifos(self):
        ...
        input_fifo = os_unicode_fs.path.join(
            self.settings["PORTAGE_BUILDDIR"], ".ipc", "in"
        )
        output_fifo = os_unicode_fs.path.join(
            self.settings["PORTAGE_BUILDDIR"], ".ipc", "out"
        )
        ...
        for p in (input_fifo, output_fifo):
            ...
            st = None
            try:
                st = os_unicode_fs.lstat(p)
            except OSError:
                os_unicode_fs.mkfifo(p)
            else:
                if not stat.S_ISFIFO(st.st_mode):
                    st = None
                    try:
                        os_unicode_fs.unlink(p)
                    except OSError:
                        pass
                    os_unicode_fs.mkfifo(p)
    ...
    def _can_log(self, slave_fd):
        ...
        ) or os_unicode_fs.isatty(slave_fd)
    ...
    def _async_waitpid_cb(self, *args, **kwargs):
        ...
        if self._ipc_daemon is not None:
            ...
            if exit_file and not os_unicode_fs.path.exists(exit_file):
                if self.returncode < 0:
                    if not self.cancelled:
                        self._killed_by_signal(-self.returncode)
                else:
