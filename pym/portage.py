...
def env_update(makelinks=1):
	global root
	if not os.path.exists(root+portage_const.PREFIX+"/etc/env.d"):
		prevmask=os.umask(0)
		os.makedirs(root+portage_const.PREFIX+"/etc/env.d",0755)
		os.umask(prevmask)
	fns=listdir(root+portage_const.PREFIX+"/etc/env.d",EmptyOnError=1)
	fns.sort()
	pos=0
	while (pos<len(fns)):
		if len(fns[pos])<=2:
	...
	for x in fns:
		...
		if x[-1]=='~' or x[-4:]==".bak":
			continue
		myconfig=getconfig(root+portage_const.PREFIX+"/etc/env.d/"+x)
		if myconfig==None:
			writemsg("!!! Parsing error in "+str(root)+portage_const.PREFIX+"/etc/env.d/"+str(x)+"\n")
			#parse error
			continue
	...
	if os.path.exists(root+portage_const.PREFIX+"/etc/ld.so.conf"):
		myld=open(root+portage_const.PREFIX+"/etc/ld.so.conf")
	...
	if (oldld!=newld):
		...
		myfd = atomic_ofstream(os.path.join(root, portage_const.PREFIX, "etc", "ld.so.conf"))
		myfd.write("# ld.so.conf autogenerated by env-update; make all changes to\n")
		myfd.write("# contents of "+portage_const.PREFIX+"/etc/env.d directory\n")
	...
	if prelink_capable:
		newprelink = atomic_ofstream(os.path.join(root, portage_const.PREFIX, "etc", "prelink.conf"))
		newprelink.write("# prelink.conf autogenerated by env-update; make all changes to\n")
		newprelink.write("# contents of "+portage_const.PREFIX+"/etc/env.d directory\n")

		for x in map(lambda x: os.path.join(portage_const.PREFIX,x), ["bin","sbin","usr/bin","usr/sbin","lib","usr/lib"]):
	...
	for x in map(lambda x: portage_const.PREFIX+x, specials["LDPATH"]+['/usr/lib','/lib']):
		try:
	...
	penvnotice += "# GO INTO "+portage_const.PREFIX+"/etc/profile NOT "+portage_const.PREFIX+"/etc/profile.env\n\n"
	cenvnotice += "# GO INTO "+portage_const.PREFIX+"/etc/csh.cshrc NOT "+portage_const.PREFIX+"/etc/csh.env\n\n"

	#create /etc/profile.env for bash support
	outfile = atomic_ofstream(os.path.join(root, portage_const.PREFIX, "etc", "profile.env"))
	...
	outfile = atomic_ofstream(os.path.join(root, portage_const.PREFIX, "etc", "csh.env"))
...
def doebuild_environment(myebuild, mydo, myroot, mysettings, debug, use_cache, tree):
	...
	mysettings["D"] = os.path.normpath(mysettings["EDEST"]+portage_const.PREFIX)
