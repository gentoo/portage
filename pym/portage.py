...
def env_update(makelinks=1):
	...
	if (oldld!=newld):
		...
		myfd=open(root+portage_const.PREFIX+"/etc/ld.so.conf","w")
	...
	if prelink_capable:
		newprelink=open(root+portage_const.PREFIX+"/etc/prelink.conf","w")
	...
	outfile=open(root+portage_const.PREFIX+"/etc/profile.env","w")
	...
	outfile=open(root+portage_const.PREFIX+"/etc/csh.env","w")
