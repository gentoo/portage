...
try:
	...
	from portage_const import VDB_PATH, PRIVATE_PATH, CACHE_PATH, DEPCACHE_PATH, \
	  ...
	  INCREMENTALS, EAPI, MISC_SH_BINARY, EPREFIX
...
def env_update(makelinks=1, target_root=None, prev_mtimes=None, contents=None):
	...
	ldsoconf_path = os.path.join(target_root + EPREFIX, "etc", "ld.so.conf")
	try:
	...
	if prelink_capable:
		newprelink = atomic_ofstream(
			os.path.join(target_root + EPREFIX, "etc", "prelink.conf"))
	...
	outfile = atomic_ofstream(os.path.join(target_root + EPREFIX, "etc", "profile.env"))
	...
	outfile = atomic_ofstream(os.path.join(target_root + EPREFIX, "etc", "csh.env"))
...
class config:
	...
	def regenerate(self,useonly=0,use_cache=1):
		...
		env_d = getconfig(
			os.path.join(self["ROOT"] + EPREFIX, "etc", "profile.env"), expand=False)
		if env_d:
			# env_d will be None if profile.env doesn't exist.
			self.configdict["env.d"].update(env_d)
...
def doebuild_environment(myebuild, mydo, myroot, mysettings, debug, use_cache, mydbapi):
	...
	mysettings["EPREFIX"]  = EPREFIX.rstrip(os.sep)
	...
	if (mydo!="depend") or not mysettings.has_key("KV"):
		mykv,err1=ExtractKernelVersion(os.path.join(myroot + EPREFIX, "usr/src/linux"))
		if mykv:
			# Regular source tree
			mysettings["KV"]=mykv
		else:
...
def merge(mycat, mypkg, pkgloc, infloc, myroot, mysettings, myebuild=None,
	...
	if not os.access(myroot+EPREFIX.lstrip("/"), os.W_OK):
		writemsg("Permission denied: access('%s', W_OK)\n" %
				(myroot+EPREFIX.lstrip("/")), noiselevel=-1)
		return errno.EACCES
...
class vardbapi(dbapi):
	...
	def counter_tick_core(self,myroot,incrementing=1,mycpv=None):
		...
		cpath=os.path.join(myroot+EPREFIX, "var/cache/edb/counter")
		changed=0
		min_counter = 0
		if mycpv:
