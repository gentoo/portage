...
def env_update(makelinks=1, target_root=None, prev_mtimes=None, contents=None):
	...
	envd_dir = os.path.join(target_root, portage_const.EPREFIX, "etc", "env.d")
	...
	ldsoconf_path = os.path.join(target_root, portage_const.EPREFIX, "etc", "ld.so.conf")
	try:
	...
	if prelink_capable:
		newprelink = atomic_ofstream(
			os.path.join(target_root, portage_const.EPREFIX, "etc", "prelink.conf"))
	...
	for lib_dir in portage_util.unique_array(specials["LDPATH"]+['usr/lib','usr/lib64','usr/lib32','lib','lib64','lib32']):
		x = os.path.join(target_root, portage_const.EPREFIX, lib_dir.lstrip(os.sep))
		try:
	...
	outfile = atomic_ofstream(os.path.join(target_root, portage_const.EPREFIX, "etc", "profile.env"))
	...
	outfile = atomic_ofstream(os.path.join(target_root, portage_const.EPREFIX, "etc", "csh.env"))
...
class config:
	...
	def _init_dirs(self):
		...
		if not os.access(self["ROOT"] + os.sep + portage_const.EPREFIX, os.W_OK):
			return
		...
		for mypath, (gid, mode, modemask) in dir_mode_map.iteritems():
			try:
				mydir = os.path.join(self["ROOT"], self["EPREFIX"], mypath)
...
def doebuild_environment(myebuild, mydo, myroot, mysettings, debug, use_cache, mydbapi):
	...
	if (mydo!="depend") or not mysettings.has_key("KV"):
		mykv,err1=ExtractKernelVersion(os.path.join(myroot, portage_const.EPREFIX, "usr/src/linux"))
		if mykv:
			# Regular source tree
			mysettings["KV"]=mykv
		else:
...
class dblink:
	...
	def __init__(self, cat, pkg, myroot, mysettings, treetype=None,
		...
		protect_obj = portage_util.ConfigProtect(myroot
				+ os.path.sep + portage_const.EPREFIX,
