...
class PackageTracker(object):
	...
	You can the add_pkg() method to add a to-be-merged package to the PackageTracker, and ``add_installed_pkg()`` to add
	an already-installed package to the package tracker. Typical use of the package tracker involves the depgraph.py
	code populating the package tracker with calls to ``add_installed_pkg()`` to add all installed packages on the
	system, and then it is initialized and ready for use. At that point, ``depgraph.py`` can use ``add_pkg()`` to add
	to-be-installed packages to the system.
	...
	* Sub-slot conflicts. It is possible to identify situations where a to-be-installed package is in a new sub-slot.
	...
	def __init__(self, soname_deps=False):
		...
		:var _cp_pkg_map: The collection of to-be-installed (not yet merged) packages. We care about conflicts in these
		 packages.
		:var _cp_vdb_pkg_map: The collection of already-installed packages.
		:var _multi_pkgs: A list of keys in ``self._cp_pkg_map`` that have potential slot and cpv conflicts.
		:var _replacing: The mechanism by which ``PackageTracker`` records to-be-installed packages that 'cover up'
		 already-installed packages. ``self._replacing[cp_key] = [ new_pkg_that_replaced_cp_key... ]``.
		:var _replaced_by: ``self.replaced_by[cp_key] == [ replaced_pkg_1, replaced_pkg_2 ]
		"""
